                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('depositos.update', $deposito->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="nombre" class="form-control" value="{{ old('nombre', $deposito->nombre) }}" required>
                        </div>
                        <div class="form-group">
                            <label class="label-negrita" for="desc">Descripción</label>
                            <input type="text" name="desc" class="form-control" value="{{ old('desc', $deposito->desc) }}">
                        </div>
                        <div class="form-group">
                            <label class="label-negrita" for="obs">Observaciones</label>
                            <input type="text" name="obs" class="form-control" value="{{ old('obs', $deposito->obs) }}">
                        </div>
                        <div class="form-group">
                            <label class="label-negrita" for="activo">Activo</label>
                            <select name="activo" id="activo" class="form-control">
                                <option value="1" {{ $deposito->activo == 1 ? 'selected' : '' }}>Activo</option>
